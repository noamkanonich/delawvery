                        ...
                        >
                            <CardContent>
                                <Typography variant="h6" mb={2} gutterBottom>
                                    הזמנה {order.id}
                                    <Box sx={{ float: "left" }}>
                                        {((toggle !== order.id) || !props.isEdited)
                                            ? (<ModeEditIcon id={order.id} color="action" sx={{ fontSize: 20 }} onClick={() => handleOrderEdit(order)} />)
                                            : (null)}
                                        <ClearIcon color="action" sx={{ fontSize: 20 }} onClick={() => { handleCleanOrder(order.id) }} />
                                    </Box>
                                </Typography>
